...
class DeformFeatureFusion(nn.Module):

    def __init__(self, input_layers_list=[128, 256, 512]):
        super(DeformFeatureFusion, self).__init__()
        ...
        for output_layer in input_layers_list:
            for input_layer in input_layers_list:
                if (input_layer == output_layer):
                    self.non_local_conv_list.append(NonLocalConv2d(input_layer, output_layer))  # 全局模块list
    ...
    def forward(self, x):
        ...
        x[0] = self.non_local_conv_list[0](x[0]) + x[0]
        x[1] = self.non_local_conv_list[1](x[1]) + x[1]
        x[2] = self.non_local_conv_list[2](x[2]) + x[2]

        out.append(x[0] +
                   self.resize(self.feature_list[0](x[1]), x[0].shape[-2:]) +
                   self.resize(self.feature_list[1](x[2]), x[0].shape[-2:]))
        out.append(x[1] +
                   self.resize(self.feature_list[2](x[0]), x[1].shape[-2:]) +
                   self.resize(self.feature_list[3](x[2]), x[1].shape[-2:]))
        out.append(x[2] +
                   self.resize(self.feature_list[4](x[0]), x[2].shape[-2:]) +
                   self.resize(self.feature_list[5](x[1]), x[2].shape[-2:]))
...
if __name__ == '__main__':
    ...
    fpn = DeformFeatureFusion(*[input_layers_list])
    x = []
    ...
    b = eval(a)
    print(b)
    print(b is DeformFeatureFusion)
    print(isinstance(b, DeformFeatureFusion))
